...
void PhysicsComponent::integrate(PositionComponent& posComponent, double elapsedTime)
{
	...
	if (elapsedTime > 0.0f)//no need to do this for no elapsed time
	{
		...
		mData.vel += (mData.acc * (float)elapsedTime);
		mData.rotVel += (mData.rotAcc * (float)elapsedTime);
		...
		posComponent.modPosition(mData.vel * (float)elapsedTime);
		posComponent.modFacing(mData.rotVel * (float)elapsedTime);
	}
